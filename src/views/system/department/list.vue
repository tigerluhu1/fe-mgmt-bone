...
export default {
  ...
  computed: {
    searchFormField () {
      return [
        { prop: 'code', label: this.$t('system_department.code'), type: 'Input', component: { clearable: true } },
        { prop: 'name', label: this.$t('system_department.name'), type: 'Input', component: { clearable: true } },
        { slotName: 'tableOperation', col: { xs: 12, sm: 16, md: 12, style: { textAlign: 'right' } }, labelWidth: '0' }
      ]
    },
    tableColumn () {
      return [
        { label: this.$t('system_department.code'), prop: 'code', width: 120 },
        { label: this.$t('system_department.name'), prop: 'name', width: 200 },
        { label: this.$t('system_department.companyName'), prop: 'companyName', width: 200 },
        { label: this.$t('system_department.status'), prop: 'status', width: 80, slotName: 'delFlag' },
        { label: this.$t('system_department.remarks'), prop: 'remarks' },
  ...
  methods: {
    handleDataSearch () {
      // 部门类型数据
      let params = Object.assign({ type: 2, category: 'sys_dept_category_1' }, this.tablePageInfo, this.searchModel)
