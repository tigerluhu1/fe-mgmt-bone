  ...
  <div class="page__wrapper">
    <simple-table :table-column="tableColumn" :table-data="tableData" :table-page-info="tablePageInfo" :table-data-count="tableDataTotalCount" @page-info-change="handlePageInfoChange">
      ...
      <template slot-scope="scope" slot="category">
        {{ scope.row.category ? scope.row.category.split(',').map(item => {
        return $getDictLabel({type: 'sys_office_category', value: item
        })}).join() : '' }}
      </template>
...
const BASEURL = {
  list: '/api/sys/office',
  delete: '/api/system/office/delete'
}
export default {
  name: 'main_office',
  mixins: [pageMixins],
  ...
  computed: {
    searchFormField () {
      return [
        { prop: 'code', label: this.$t('system_office.code'), type: 'Input', component: { clearable: true } },
        { prop: 'name', label: this.$t('system_office.name'), type: 'Input', component: { clearable: true } },
        { prop: 'category', label: this.$t('system_office.category'), type: 'Select', component: { optionList: this.category, clearable: true } },
        { slotName: 'tableOperation', col: { xs: 24, sm: 8, md: 6, style: { textAlign: 'right' } }, labelWidth: '0' }
      ]
    },
    ...
    tableColumn () {
      return [
        ...
        { prop: 'delFlag', label: this.$t('system_office.delFlag'), width: 80, slotName: 'delFlag' },
